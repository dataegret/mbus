...
AS $$
begin
	delete from mbus.trigger where dst like 'temp.%' and not exists (select * from pg_stat_activity where dst like 'temp.' || md5(procpid::text || backend_start::text) || '%');
	delete from mbus.tempq where not exists (select * from pg_stat_activity where (headers->'tempq') is null and (headers->'tempq') like 'temp.' || md5(procpid::text || backend_start::text) || '%');
end;
$$;
...
create function queue_acl( oper text, usr text, qname text, viewname text, schemaname text)
returns void 
language plpgsql as
$_$
declare
