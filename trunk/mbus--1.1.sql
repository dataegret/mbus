...
CREATE FUNCTION readme_rus() RETURNS text
 ...
 Очередь создается функцией
  ...
  select mbus.post_<qname>(data hstore, 
   ...
   В настояшей реализации доступны только как постоянные подписчики (durable subscribers), так и временные. 
   Постоянный подписчик создается функцией 
    ...
    Получение сообщений подписчиком:
     ...
     Временный подписчик создается функцией select mbus.create_temporary_consumer(qname),
     или mbus.create_temporary_consumer(qname, selector), которая возвращает имя временной оченеди,
     куда и будут помещены копии сообщений. Следует иметь в виду, что это действительно копии,
     и, кроме того, в случае использования этого функционала должна периодически проводится очистка
     путем вызова функции mbus.clear_tempq()
...
begin
 ...
 for r in select * from mbus.queue loop
   ...
   msg_exists_qry := msg_exists_qry || 'when $1 like $LIKE$' || lower(r.qname) || '.%$LIKE$ then exists(select * from mbus.qt$' || r.qname || ' q where q.iid=$1 and cons_id <>all(q.received) and (exist(q.headers,$$is_zombie$$) or coalesce(q.expires,$$infinity$$)>now()))'||chr(10);
